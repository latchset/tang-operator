...
# example.com/operator-test-bundle:$VERSION and example.com/operator-test-catalog:$VERSION.
IMAGE_TAG_BASE ?= example.com/operator-test
...
$(CONTROLLER_GEN): $(LOCALBIN)
	...
	if [ -n "${GOARCH}" ]; then \
	  if [ -n "${TANG_OPERATOR_CI}" ]; then \
	    GOARCH= go install sigs.k8s.io/controller-tools/cmd/controller-gen@$(CONTROLLER_TOOLS_VERSION); \
	  else \
